...
public class TrackService {
	 ...
	 * 
	 */
	private static Logger LOGGER = LoggerFactory.getLogger(TrackService.class);
	 ...
	 * @return
	 */
	public static DataMap listTracks(String aDirPath, String aDirName) {
		String path = StringUtils.isEmpty(aDirPath) ? "~" : aDirPath;
		String name = StringUtils.isEmpty(aDirName) ? "" : aDirName;
		File targetDir = new File(path, name);

		LOGGER.debug("Fetching tracks for [{}]", targetDir);

		DataMap dataMap = new DataMap();
		dataMap.put("dirPath", targetDir.getPath());
